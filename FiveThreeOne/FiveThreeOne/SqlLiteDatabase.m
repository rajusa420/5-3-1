...
- (BOOL)bindInt64: (sqlite_int64) value forStatement: (sqlite3_stmt*) statement atIndex: (int) index
{
    ...
    {
#if LOG_SQL_LITE_ERRORS
        NSLog(@"Sql: failed to bind int64");
#endif
        return NO;
    }
...
-(BOOL) bindDouble: (double) value forStatement: (sqlite3_stmt*) statement atIndex: (int) index
{
    ...
    {
        #if LOG_SQL_LITE_ERRORS
                NSLog(@"Sql: failed to bind double");
        #endif
        return NO;
    }
...
{
    int result = sqlite3_bind_text(statement, index, [text UTF8String], -1, SQLITE_STATIC);
    if (result != SQLITE_OK)
    {
#if LOG_SQL_LITE_ERRORS
        NSLog(@"Sql: failed to bind string");
#endif
        return NO;
    }
...
{
    int result = sqlite3_bind_blob(statement, index, &uuid, sizeof(uuid), SQLITE_STATIC);
    if (result != SQLITE_OK)
    {
#if LOG_SQL_LITE_ERRORS
        NSLog(@"Sql: failed to bind CFUUID");
#endif
        return NO;
    }
...
{
    const char* bytes = sqlite3_column_blob(statement, column);

    if (bytes == nil)
    {
        return [CFUUIDHelper createCFUUID];
    }

    return [CFUUIDHelper CFUUIDwithBytes: bytes];
}
